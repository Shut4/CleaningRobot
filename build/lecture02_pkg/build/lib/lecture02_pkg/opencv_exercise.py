...
class ImageConverter(Node):
    ...
    def __init__(self):
        ...
        self.image_sub = self.create_subscription(
            Image,  # メッセージ型
            "image_raw",  # トピック名
            self.callback,  # コールバックメソッド
            10,  # キューサイズ
        )

        # TODO: パブリッシャの宣言
        self.image_pub = self.create_publisher(msg_type=Image, topic="image_topic_2", qos_profile=10)
    ...
    def callback(self, msg):
        ...
        ros_img = self.bridge.cv2_to_imgmsg(cvim=cv_image,encoding="bgr8")
